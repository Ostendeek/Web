...
public class App {
    ...
    private static void showTeam(){ 
        ...
        try {
            ...
            ClientResponse response = webResource.accept(MediaType.APPLICATION_JSON + ";charset=utf-8").header("Login","user").header("Pass","user").get(ClientResponse.class);            
    ...
    private static void insertTeam() {
        try {
            ...
            WebResource webResource = client.resource(URL);
            webResource = webResource.queryParam("name",URLEncoder.encode(jsonString, "UTF-8"));                                                         
            ClientResponse response = webResource.accept(MediaType.APPLICATION_JSON + ";charset=utf-8").header("Login","user").header("Pass","user").put(ClientResponse.class);           
            if (response.getStatus() != ClientResponse.Status.OK.getStatusCode()) {
                throw new IllegalStateException(response.getEntity(String.class));
            }
    ...
    private static void updateTeam() {
        try {
            ...
            WebResource webResource = client.resource(URL);
            webResource = webResource.queryParam("name",URLEncoder.encode(jsonString, "UTF-8"));
            ClientResponse response = webResource.accept(MediaType.APPLICATION_JSON + ";charset=utf-8").header("Login","user").header("Pass","user").post(ClientResponse.class);
            if (response.getStatus() != ClientResponse.Status.OK.getStatusCode()) {
                throw new IllegalStateException(response.getEntity(String.class));
            }
            GenericType<String> type = new GenericType<String>() {};
    ...
    private static void deleteTeam(){
        try {
            ...
            ClientResponse response = webResource.accept(MediaType.APPLICATION_JSON + ";charset=utf-8").header("Login","user").header("Pass","user").delete(ClientResponse.class);
            if (response.getStatus() != ClientResponse.Status.OK.getStatusCode()) {
                throw new IllegalStateException(response.getEntity(String.class));
            }
            GenericType<String> type = new GenericType<String>() {};
